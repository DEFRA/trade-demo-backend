 ...
 * <p>
 * Generates a CHED reference based on the notification ID without making actual API calls. This
 * will be replaced with a real implementation once IPAFFS integration is ready.
 */
@Component
@Slf4j
...
public class MockIpaffsNotificationClient implements IpaffsNotificationClient {
     ...
     * <p>
     * Transforms the notification ID into a CHED reference: CDP.2025.12.05.6 ->
     * CHEDA.2025.12050600
     *
     ...
     */
    @Override
    public ResponseEntity<String> submitNotification(
        ...
        log.debug("INS-ConversationId header would be set to: {}", conversationId);
     ...
     * <p>
     * Transforms: CDP.YYYY.MM.DD.S -> CHEDA.YYYY.MMDDSSSS Example: CDP.2025.12.05.6 ->
     * CHEDA.2025.12050600
     *
    ...
    private String generateChedReference(String notificationId) {
        ...
        if (parts.length != 5) {
            throw new IllegalArgumentException(
                "Invalid notification ID format. Expected: CDP.YYYY.MM.DD.S, got: "
                    + notificationId);
        }
